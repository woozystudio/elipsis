    ...
    async Execute(interaction: ChatInputCommandInteraction) { 
        if(interaction.isButton()) {
            ...
            if(buttons.guild && buttons.member && buttons.channel && buttons.memberPermissions ) {
                if(!["close"].includes(buttons.customId)) return;
                ...
                if(data) {
                    switch (buttons.customId) {
                        case "close":
                            ...
                            if (data.Closed === true) return buttons.reply({ content: `${Symbols.Success} This application is already getting deleted...`, ephemeral: true });
