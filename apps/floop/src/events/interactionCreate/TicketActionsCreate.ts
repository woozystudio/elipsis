            ...
            if(buttons.guild && buttons.member && buttons.channel) {
                ...
                if(data) {
                    switch (buttons.customId) {
                        case "close":
                            if (data.Closed === true) return buttons.reply({ content: `✅ \`|\` This ticket is already getting deleted...`, ephemeral: true });
                            ...
                            buttons.reply({ content: `✅ \`|\` Your ticket is being prepared for closing.` });
                            break;
